...
class Reinforce:
    ...
    def _get_returns(self, r, mask, terminated, max_episode_len):
        r = r.squeeze(-1)  # r为一个episode中每个step所获得的奖励
        mask = mask.squeeze(-1)
        ...
        # 可能是因为所有智能体的网络结构一致所以直接认为所有智能体的状态价值一样？
    ...
    def _get_action_prob(self, batch, max_episode_len, epsilon):
        ...
        action_num = avail_actions.sum(dim=-1, keepdim=True).float().repeat(1, 1, 1, avail_actions.shape[-1])   # 可以选择的动作的个数 (episode_num, max_episode_len, n_agents, n_actions)
    ...
    def save_model(self, run_num, train_step):
        ...
        torch.save(self.eval_rnn.state_dict(),  self.model_dir + '/' + str(run_num) + "_" + num + '_rnn_params.pkl')