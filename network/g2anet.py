...
class G2ANet(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.gnn = GCNConv(args.rnn_hidden_dim + args.attention_dim, args.n_actions)
        self.args = args
        self.input_shape = input_shape

    def forward(self, obs, hidden_state):
        size = obs.shape[0]  # batch_size * n_agents
        # 先对obs编码, obs: (batch_size * n_agents, input_shape)
        obs_encoding = f.relu(self.encoding(obs))  # obs_encoding: (batch_size * n_agents, rnn_hidden_dim)
        h_in = hidden_state.reshape(-1, self.args.rnn_hidden_dim)  # h_in: (batch_size * n_agents, rnn_hidden_dim)
        ...
        if self.args.hard:
            ...
            for i in range(self.args.n_agents):
                ...
                h_hard_i = torch.stack(h_hard_i, dim=0)  # h_hard_i: (n_agents - 1, batch_size, rnn_hidden_dim * 2)
                input_hard.append(h_hard_i)
            # i循环结束之后，input_hard是一个list里面装着n_agents个维度为(n_agents - 1, batch_size, rnn_hidden_dim * 2)的tensor
            input_hard = torch.stack(input_hard, dim=-2)  # input_hard: (n_agents - 1, n_agents, batch_size, rnn_hidden_dim * 2)
            ...
            # 2*3*128 (num_layers * num_directions, batch_size * n_agents, hidden_size), num_directions represent the RNN directions number
            h_hard = torch.zeros((2 * 1, size, self.args.rnn_hidden_dim))  # 因为是双向GRU，每个GRU只有一层，所以第一维是2 * 1
            # 2*3*64
            if self.args.cuda:
                h_hard = h_hard.cuda()
            h_hard, hidden = self.hard_bi_GRU(input_hard, h_hard)  # (n_agents - 1, batch_size * n_agents, rnn_hidden_dim * 2)
            h_hard = h_hard.permute(1, 0, 2)  # (batch_size * n_agents, n_agents - 1, rnn_hidden_dim * 2) swap the first two dimension
            ...
            hard_weights = self.hard_encoding(h_hard)  # after encoding; (batch_size * n_agents * (n_agents - 1), rnn_hidden_dim * 2)
            hard_weights = f.gumbel_softmax(hard_weights, tau=0.01)
            # print(hard_weights)
            hard_weights = hard_weights[:, 1].view(-1, self.args.n_agents, 1, self.args.n_agents - 1)  # (batch_size, n_agents, 1, n_agents - 1)
            hard_weights = hard_weights.permute(1, 0, 2, 3)  # (n_agents, batch_size, 1, n_agents - 1) swap the first two dimension
        ...
        for i in range(self.args.n_agents):
            ...
            v_i = torch.stack(v_i, dim=0)  # (n_agents - 1, batch_size, args.attention_dim)
            v_i = v_i.permute(1, 2, 0)  # swap the three dimension, (batch_size, args.attention_dim, n_agents - 1)
            # (batch_size, 1, attention_dim) * (batch_size, attention_dim，n_agents - 1) = (batch_size, 1，n_agents - 1)
            score = torch.matmul(q_i, k_i)

            # 归一化 (batch_size, 1, n_agents - 1)
        ...
        final_input = torch.cat([h_out, x], dim=-1)  # 3*96 (batch_size * n_agents, args.rnn_hidden_dim + args.attention_dim)
        """
        x = torch.tensor([final_input[i].tolist() for i in range(self.args.n_agents)], dtype=torch.float)
        edge_index = list(itertools.permutations([i for i in range(self.args.n_agents)], 2))
        for i in range(len(edge_index)):
            edge_index[i] = list(edge_index[i])
        edge_index = torch.tensor(edge_index, dtype=torch.long)
        if self.args.cuda:
            x = x.cuda()
            edge_index = edge_index.cuda()
        data = Data(x=x, edge_index=edge_index.t().contiguous())        
        output = self.decoding(data.x, data.edge_index)
        """
        output = self.decoding(final_input)

        return output, h_out

# def geonet(self, input_size, output,):
#     # self.decoding = nn.Linear(args.rnn_hidden_dim + args.attention_dim, args.n_actions)
#     gnn = GCNConv(self.args.rnn_hidden_dim + self.args.attention_dim, self.args.n_actions)
#
#     return output
# def convert_to_data(self, input):
#